...
cli = spotipy.Spotify(auth_manager=auth)
...
PREP_FINISHED = False
async def prep_CurPlaying(refresh:bool=False):
    t=Toplevel()
    t.title("Loading")
    tf = Frame(t)
    tf.grid()
    Label(tf,text="Loading your currently playing data...").grid()
    pb=Progressbar(tf,mode='indeterminate')
    pb.start()
    pb.grid(sticky='we')
    waiting = refresh
    while True:
        if not waiting:
            show_CurrentlyPlaying()
            waiting = True
        root.update()
        if PREP_FINISHED:
            t.destroy()
...
Button(rootFrame,text="View my currently playing",command=async_handler(prep_CurPlaying)).grid(row=1,column=1)
